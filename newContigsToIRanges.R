    ...
    for(i in 1:length(contigs)){
        ...
        for(n in seq(1,(length(contigs[[i]][[1]])-2),3)){
            for(m in 1:length(contigs[[i]][[1]][[n+1]])){
                comp[[m]] = IRanges(start = contigs[[i]][[1]][[n+1]][m],end = contigs[[i]][[1]][[n+2]][m])
                j = j +1
            }
        }
        ...
        if(length(contigs[[i]]) > 1){
            ...
            for(n in seq(1,(length(contigs[[i]][[2]])-2),3)){
                for(m in 1:length(contigs[[i]][[2]][[n+1]])){
                    cont[[m]] = IRanges(start = contigs[[i]][[2]][[n+1]][m],end = contigs[[i]][[2]][[n+2]][m])
                    j = j +1
                }
...
pfamsPerContig <- function(pfams,catalogue,minContigLength,meanContigLength,number,comp = c(0.6,1),cont = c(0,0.4),seed =1){
    x = Sys.time()
    res = list()
    
    contigs = getContigsAsIRanges3(pfams,catalogue,minContigLength ,meanContigLength,comp,cont,number,seed)
    print(head(contigs))
